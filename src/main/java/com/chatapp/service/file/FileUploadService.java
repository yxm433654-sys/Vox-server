...
import com.chatapp.service.media.DynamicPhotoService;
import com.chatapp.service.media.MediaProbeService;
import com.chatapp.service.media.MediaSourceTypes;
import com.chatapp.service.media.VideoCoverService;
...
public class FileUploadService {
    ...
    private final DynamicPhotoService dynamicPhotoService;
    ...
    public FileUploadResponse uploadFile(MultipartFile file, Long userId) throws Exception {
        ...
        try {
            ...
            if (saved.getFileType() == FileResource.FileType.VIDEO) {
                coverResource = videoCoverService.createPlaceholderAndSchedule(saved, file.getOriginalFilename(), userId);
            }
            return fileResponseMapper.toUploadResponse(saved, coverResource);
        } finally {
            tempFile.delete();
        }
    }
    ...
    public FileUploadResponse uploadLivePhoto(MultipartFile jpeg, MultipartFile mov, Long userId) throws Exception {
        ...
        dynamicPhotoService.scheduleLivePhotoProcessing(
                jpegFile.getAbsolutePath(),
                movFile.getAbsolutePath(),
                savedCover.getId(),
                savedVideo.getId()
        );
        return fileResponseMapper.toDynamicUploadResponse(savedCover, savedVideo);
    }
    ...
    public FileUploadResponse uploadMotionPhoto(MultipartFile file, Long userId) throws Exception {
        ...
        dynamicPhotoService.scheduleMotionPhotoProcessing(
